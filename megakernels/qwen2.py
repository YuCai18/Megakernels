...
def _head_dim(config: Qwen2Config) -> int:
...
class Qwen2Attention(nn.Module):
    def __init__(
        self, config: Qwen2Config, extra_config: ExtraModelConfig, layer_idx: int
    ):
        super().__init__()
        self.config = config
        ...
        # Qwen2 uses bias=True for q/k/v projections (differs from Llama)
...
class Qwen2Block(nn.Module):
    def __init__(
        ...
        self.self_attn = Qwen2Attention(config, extra_config, layer_idx)
        self.mlp = Qwen2MLP(config, extra_config, layer_idx)
...
class Qwen2LMHead(nn.Module):
    def __init__(self, config: Qwen2Config, extra_config: ExtraModelConfig):
        super().__init__()
        self.config = config
        self.extra_config = extra_config
...
class Qwen2Model(nn.Module):
    ...
    def __init__(
        ...
        self.config = config
        self.extra_config = extra_config
        self.embed_tokens = Qwen2Embeddings(config)

        layers = []
        for i in range(config.num_hidden_layers):
            layers.append(Qwen2Block(config, extra_config, i))

        self.layers = nn.ModuleList(layers)

        self.rope = Qwen2RotaryEmbedding(
            config=config,
        )
...
class Qwen2ForCausalLM(nn.Module):
    def __init__(
        ...
        self.model = Qwen2Model(config, extra_config)

        self.lm_head = Qwen2LMHead(config, extra_config)
    ...
    def setup_caches(self):
        ...
        for layer_idx in range(self.config.num_hidden_layers):
            layer: Qwen2Block = self.model.layers[layer_idx]  # type: ignore
    ...
    def from_pretrained(
        ...
        config: Qwen2Config = Qwen2Config.from_pretrained(model_name_or_path)  # type: ignore
    ...
    def stack_params(self):
        ...
        layers: list[Qwen2Block] = self.model.layers  # type: ignore
        ...
        for self_attn in self_attns:
            ...
            # Qwen2 has bias in q/k/v projections
            cat_bias = torch.cat(
                [
        ...
        for i, self_attn in enumerate(self_attns):
            ...
            q_weight, k_weight, v_weight = qkv_weight.split(
                [
                    self.config.num_attention_heads * _hd,
                    self.config.num_key_value_heads * _hd,
                    self.config.num_key_value_heads * _hd,
                ],
                dim=0,
            )
